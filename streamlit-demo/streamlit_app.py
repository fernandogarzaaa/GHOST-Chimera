...
st.subheader("A Vultr-ready enterprise agent platform for creating role-specific Ghost operators")
st.write(
    "Ghost Chimera creates a customizable AI operator proxy trained on a user's approved "
    "context, tools, workflows, and policy boundaries. This Streamlit page is a safe judge "
    "landing demo; the production-style runtime is the Vultr-hosted Ghost Console."
)
...
st.code(
    f"""ghostchimera path set --profile "{role.lower().replace(" ", "-")}" \\
