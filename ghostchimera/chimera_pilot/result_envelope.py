"""Result envelope extracted from ChimeraLang.

Universal result transport with confidence, provenance, sources, claims,
warnings, constraints, and metadata. Standalone (stdlib-only).
"""
...
def merge_envelopes(
    ...
    *,
    ...
    """Merge multiple envelopes using a confidence-weighted or simple strategy."""
    if not envelopes:
    ...
    merged = ResultEnvelope(
        ...
        metadata={
            "merged_from": [env.envelope_id for env in envelopes],
            "merge_value_mode": merge_value_mode,
        },
    )
