"""Persistent namespace store extracted from ChimeraLang.

Namespace-scoped JSON persistence with atomic writes (write to .tmp then
rename), append-with-capping, and path sanitization. Standalone (stdlib-only).
"""
...
class PersistentNamespaceStore:
    """Path-scoped JSON storage keyed by (kind, namespace)."""
