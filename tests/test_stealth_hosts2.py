...
def test_codex_plugin_and_context_block() -> None:
    ...
    try:
        ...
        _train(adapter)
        decision = adapter.observe_session(
            {"kind": "prompt", "session_id": "s", "relevance": 0.95, "confidence": 0.93, "benefit": 0.9}
        )
...
def test_gemini_extension_hook() -> None:
    ...
    try:
        ...
        _train(adapter)
        out = adapter.handle_hook_input({"eventName": "before_prompt", "sessionId": "s", "prompt": "work on Moovsoon"})
        assert "contextOut" in out and "Moovsoon" in out["contextOut"]
        out2 = adapter.handle_hook_input({"eventName": "before_prompt", "sessionId": "s", "prompt": "/ghost status"})
...
def test_hermes_prefetch_and_report() -> None:
    ...
    try:
        ...
        _train(adapter)
        context = adapter.prefetch({"session_id": "s", "relevance": 0.95, "confidence": 0.93, "benefit": 0.9})
        ...
        assert loop.graph.workflow_score(loop.interventions[result.intervention_id].workflow) > 0.5
