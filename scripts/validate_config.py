...
def validate_config(
    ...
    results["checks"].append({
        "name": "GHOSTCHIMERA_CONSOLE_AUTH_TOKEN",
        "value": redact_secret(console_token),
        "status": token_status
    })
    ...
    results["checks"].append({
        "name": "VULTR_INFERENCE_API_KEY",
        "value": redact_secret(vultr_key),
        "status": vultr_key_status
    })
...
Examples:
  ...
  python scripts/validate_config.py --format json
    ...
    parser.add_argument(
        "--production",
        action="store_true",
        help="Enable production mode (strict validation)"
    )
