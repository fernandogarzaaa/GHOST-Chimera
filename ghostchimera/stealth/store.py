...
class StealthStore:
        ...
        with self._lock:
            self._conn.execute(
                ...
                (
                    d["event_id"],
                    d["event_type"],
                    float(d.get("timestamp", time.time())),
                    str(d.get("source", "")),
                    str(d.get("actor", "")),
    ...
    def recent_events(
        self, *, event_type: str | None = None, session_id: str | None = None, limit: int = 100
    ) -> list[dict[str, Any]]:
        query = (
            "SELECT event_id, event_type, timestamp, source, actor, payload, provenance,"
        ...
        with self._lock:
            if d is None:  # duck-typed mapping
                ...
                self._conn.execute(
                    ...
                    (
                        d["id"],
                        d.get("trigger_event_id", ""),
                        d.get("workflow", ""),
                        float(d.get("confidence", 0.0)),
                        str(d.get("state", "created")),
                        str(d.get("outcome", "pending")),
                        str(d.get("reason", "")),
                        json.dumps(d.get("provenance") or {}),
                        float(d.get("created_at", now)),
                        now,
                    ),
                )
            self._conn.commit()
    ...
    def record_outcome(self, intervention_id: str, workflow: str, outcome: str) -> None:
        ...
        with self._lock:
            self._conn.execute(
                "INSERT INTO outcomes(intervention_id, workflow, outcome, recorded_at) VALUES(?,?,?,?)",
                (intervention_id, workflow, outcome, now),
            )
            self._conn.execute(
                "UPDATE interventions SET outcome = ?, state = 'outcome', updated_at = ? WHERE id = ?",
    ...
    def useful_intervention_rate(self, workflow: str | None = None) -> float:
        ...
        with self._lock:
            total = int(self._conn.execute(f"SELECT COUNT(*) FROM outcomes{where}", params).fetchone()[0])
            useful = int(
                self._conn.execute(
    ...
    # -- workflows ------------------------------------------------------------
    def upsert_workflow(
        self, name: str, pattern: list[str], support: int, confidence: float, *, explicit: bool = False
    ) -> None:
        with self._lock:
            self._conn.execute(
    ...
    def load_workflows(self) -> list[dict[str, Any]]:
        with self._lock:
            rows = self._conn.execute(
                "SELECT name, pattern_json, support, confidence, explicit FROM workflows"
            ).fetchall()
        return [
            {"name": r[0], "pattern": json.loads(r[1]), "support": r[2], "confidence": r[3], "explicit": bool(r[4])}
            for r in rows
        ]
